...
export default {
  data: () => ({
    ...
    dice1roll: "",
    dice2roll: "",
    playImageArr: ["panda", "cat", "pig", "monkey", "bear", "dog", "owl"],
    currentTurn: -1
  }),
  ...
  methods: {
    ...
    currentPlayerTurn() {
      // debugger;
      this.currentTurn++;
      if (this.currentTurn > this.$store.state.players.length) {
        this.currentTurn = 0;
      }
      console.log(this.$store.state.players[this.currentTurn]);
      return this.$store.state.players[this.currentTurn];
    }
  },
  ...
  mounted() {
    ...
    setInterval(() => {
      ...
      let pid = this.currentPlayerTurn();
      console.log(pid);

      Promise.all([
        this.diceRoll(dice1, this.dice1roll).finished,
        this.diceRoll(dice2, this.dice2roll).finished
      ]).then(() => {
